...
module.exports = {
  ...
  try {
    ...
    const cartProduct = await this.findCartProductByCartAndProduct(existingCart, product);
    let cart;


    if (!existingCart) { /* si no existe carrito */
    ...
    } else { /* si existe carrito */
      cart = existingCart;


      if (cartProduct && cartProduct.product_id == product.id) { /* si existe carrito y producto */
      ...
      } else { /* si existe carrito pero NO producto */
        await CartProduct.create({
          cart_id: cart.id,
          product_id: product.id,
          quantity: quantity,
      ...
      await Cart.update(
        {
          quantity: await CartProduct.sum("quantity", {where: {cart_id: cart.id}}), 
          total_price: await CartProduct.sum("total_price", {where: {cart_id: cart.id}}),
        },
        {
